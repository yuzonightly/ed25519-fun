...
pub fn precompute_single(B: P3) -> [[Precomp; 8]; 32] {
    ...
    for i in 0..32 {
        let mut Bij = Bi;
        for j in 0..8 {
            let recip: FieldElement = Bij.Z.invert();
            let x: FieldElement = Bij.X * recip;
            let y: FieldElement = Bij.Y * recip;
            ...
            Bij = (Bij + Bi.to_Cached()).to_P3();
        }
        Bi = (Bi + Bi.to_Cached()).to_P3();
    }
...
pub fn precompute_double(B: P3) -> [Precomp; 8] {
    ...
    let mut Bi = B;
    let B2 = (Bi + Bi.to_Cached()).to_P3();
    for i in 0..8 {
        let recip = Bi.Z.invert();
        let x = Bi.X * recip;
        let y = Bi.Y * recip;

        precomp[i] = Precomp {
            YpX: FieldElement::reduce((y + x).0),
            YmX: FieldElement::reduce((y - x).0),
            XY2d: FieldElement::reduce((x * y * D2).0),
        };

        Bi = (Bi + B2.to_Cached()).to_P3();
    }
...
// [i][j] = (j + 1) * 256^i * B
pub const PRECOMP_BASE: [[Precomp; 8]; 32] = [
    //Auto generated file.
    [
        Precomp {
            YpX: FieldElement([
